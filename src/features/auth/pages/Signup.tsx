...
export default function SignupPage() {
  ...
  const onSubmit = async (data: SignupData) => {
    ...
    try {
      ...
      console.log("signup", { name: data.name, email: data.email })
      await new Promise((r) => setTimeout(r, 800))
      // e.g. router.push("/welcome")
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
  ...
  const signUpWithX = async () => {
    setLoading(true)
    try {
      console.log("signup with X / Twitter")
      await new Promise((r) => setTimeout(r, 600))
    } finally {
      setLoading(false)
    }
      ...
      <div className="w-full max-w-md">
        <Card>
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Sign up with Google or X, or create an account with your email
            </CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className={cn("flex flex-col gap-6")}>
              <FieldGroup>
                ...
                <Field>
                  ...
                  {errors.name ? (
                    <FieldDescription className="text-destructive text-sm">{errors.name.message}</FieldDescription>
                  ) : null}
                </Field>
                ...
                <Field>
                  ...
                  {errors.confirmPassword ? (
                    <FieldDescription className="text-destructive text-sm">{errors.confirmPassword.message}</FieldDescription>
                  ) : null}
                </Field>
